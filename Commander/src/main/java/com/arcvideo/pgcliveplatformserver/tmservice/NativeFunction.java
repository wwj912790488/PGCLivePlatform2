...
public class NativeFunction {
	private static Logger logger = Logger.getLogger(NativeFunction.class.getName());
    ...
    public static long FileCacheMaxSize = 100L*1024*1024; // default value if no setting in ASCodec.ini
    public static long FileCacheAdvance = 128L*1024;
    public static int ChunkedTransfer = 1;	// 0: chunked transfer is not allowed, 1: allowed.
    public static long PresumedTotalBytes = 1125899906842624L; // default value is 1T if no setting in ASCodec.ini
    public static int ReadTimeout = 15000; // milliseconds loaded from ini
    
  //loaded from tmservice.properties
    public static String dumpFile;
    public static Integer socketRecvBufferSize;
	...
	private static void InitializePath() {
    	try {
    		...
    		if (OS != null && OS.indexOf("win") >= 0) {
    			...
    			dumpFile = pps.getProperty("dump_file");
    			
    			String srbs = pps.getProperty("socket_recv_buffer_size");
    			if (srbs != null) {
    				socketRecvBufferSize = Integer.parseInt(srbs);
    			}
    ...
    public static void main(String[] args) {
    	...
    	FileInputStream streamData = null;
    	try {
			...
			streamData = new FileInputStream(fileData);
