    ...
    var handleDatatable = function (dataTable, columns, urlList) {
        ...
        var tableClient = dataTable.DataTable({
            ...
            "fnServerParams": function (aoData) {
                var contentId = $("#alert-form").find("[name = contentId]").val();
                var type = $("#alert-form").find("[name = type]").val();
                var serverType = $("#alert-form").find("[name = serverType]").val();
                var level = $("#alert-form").find("[name = level]").val();
                var keyword = $("#alert-form").find("[name = keyword]").val();
                var startTime =  $('#alert-form').find('[name = startTime]').val();
                var endTime =  $('#alert-form').find('[name = endTime]').val();
                aoData.push(
        ...
        $("#alert-form").find("[name = type], [name = serverType], [name = level]").on('change', function() {
            tableClient.ajax.reload();
            $('#select_all').prop('checked', false);
        })
        ...
        $('#remove-button').click(function () {
            ...
            bootbox.confirm({message: "确定要删除告警吗？", callback: function(result) {
                if (result == true) {
                    $.ajax({
                        url: "/alert/history/remove",
                        type: 'POST',
                        dataType: 'json',
        ...
        var startTimepicker =  $('#alert-form').find('[name = startTime]');
        var endTimepicker =  $('#alert-form').find('[name = endTime]');
