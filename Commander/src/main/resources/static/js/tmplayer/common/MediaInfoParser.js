...
var MEDIAINFO_OUTPORTS = "OutPorts";
var MEDIAINFO_TYPE = "type";

var MEDIAINFO_TYPE_SDI = "SDI";
...
function MediaPortParser() {
	...
	this.GetPortArray = function() {
		...
		var $ports = $(MEDIAINFO_PORTS, this.$rootNode);
		if($ports == null) return null;
		
		var arr = [];
		$(MEDIAINFO_PORT, $ports).each(function(i) {
			var obj = {};
			obj.key = $(this).attr(MEDIAINFO_INDEX);
	...
	this.GetOutPortArray = function() {
		...
		if (type == MEDIAINFO_TYPE_SDI) {
			$ports = $(MEDIAINFO_OUTPORTS, this.$rootNode);
		} else {
			$ports = $(MEDIAINFO_PORTS, this.$rootNode);
		}
...
function ValidateInputInData(inputInfo, data) {
	...
	if(inputInfo.type == INPUT_TYPE_SDI
			|| inputInfo.type == INPUT_TYPE_SDI4K
