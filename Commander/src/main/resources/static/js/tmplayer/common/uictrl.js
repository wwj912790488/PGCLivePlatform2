 ...
 *js/controls/Background.js is recommand
 * **/
...
function LoadTMPlayer(dom) {
	...
	'codebase="tmplayer/TMPlayer.CAB#Version=2,0,0,106"'+
	'id="ArcSoft_TMPlayer"'+
...
function LoadFakePlayer(dom) {
	dom.innerHTML = "<div id='FakePlayer' style='width: 480px; height: 360px; text-align:center;line-height:360px'>" + str_common.player_only_support_ie32 + "</div>";
}
...
/**
 * 
 * @param $dom jquery objets array
 * @param value
 */
function setRadio(jObjects, value)
{
    for(i = 0; i < jObjects.length; i++){
        if(jObjects.get(i).value == value){
        	jObjects.get(i).checked = true;
        	return;
        }
    }
}
